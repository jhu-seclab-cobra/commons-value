...
- `compareTo` is inconsistent with `equals`: `IntVal(1L)` and `FloatVal(1.0)` compare as equivalent but are not equal. Sorted collections (`TreeSet`, `sorted()`) collapse them into one element; hash collections keep both.

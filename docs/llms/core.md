...
when (val v: IValue = list[0]) {
    ...
    is NullVal -> null
    is Unsure -> v.core
    is NumVal -> v.toInt()     // deprecated -- use IntVal/FloatVal
    is ListVal -> v.size
    is SetVal -> v.size
    is MapVal -> v.size
    is RangeVal -> v.first
...
- **`NumVal(core: Number)`** -- **Deprecated.** Use `IntVal` for integers and `FloatVal` for floating-point. Numeric IR value. Default constructor creates `0`. Methods: `toInt()`, `toDouble()`, `toFloat()`, `toLong()`, `toShort()`, `compareTo(Int)`. Properties: `isInt`, `isLong`, `isShort`, `isByte`, `isFloat`, `isDouble`, `isPrimitiveIntegerType`, `isPrimitiveFloatingType`. Static: `NumVal.truncate(NumVal): NumVal`.
...
- **`RangeVal(start: NumVal, endInclusive: NumVal)`** -- Numeric range. Constructor: `RangeVal(Number, Number)`. Properties: `first: Number`, `last: Number`. Operators: `contains(Number)`, `contains(NumVal)`, `contains(IntVal)`, `contains(RangeVal)`, `plus(RangeVal)`. Infix: `before(RangeVal)`, `after(RangeVal)`. Methods: `map {}`.
...
- `NumVal` is deprecated; use `IntVal` for integers and `FloatVal` for floating-point.
